...
def main(_):
  ...
  with tf.Graph().as_default():
       ...
       with tf.Session() as sess:
         ...
         weight_file = '.../path/to/load/fine_tunned/cnn/weights/in/.npz'
         ...
         for epoch in range(params.No_of_Epochs):
           ...
           for y in range(m_validation):
             ...
             mean_difference = labels - mean_prediction
             mean_difference = np.absolute(mean_difference)
             mean_one_diff = 1 - mean_difference
             mean_sum_1 = np.sum(mean_one_diff)
             mean_mean_accuracy = mean_sum_1/params.NUM_CLASSES
             Predicted_video_mean[y] = mean_mean_accuracy
