# Interpretable cnn for big five personality traits using audio #
# cam cnn using raw audio #
...
def main(_):
    with tf.Graph().as_default():
        ...
        # Output layer.
        logits = tf.contrib.layers.fully_connected(net,5,activation_fn=None,    
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(param.No_of_Epochs):
                ...
                for i in range(len(train_features)):
                    ...
                    t_label = np.reshape(t_label,[1,5])
                    pred,_,loss = sess.run([logits,train_op,loss_tensor],feed_dict={ x: t_input, y: t_label})
                ...
                print('Testing...')
                rand_array_1 = [i for i in range(len(test_features))]
                np.random.shuffle(rand_array_1)
                for j in range(len(test_features)):
                    index = rand_array_1[j]
                    t_input = test_features[index]
                    t_label = test_labels[index]
                    t_label = np.reshape(t_label,[1,param.NUM_CLASSES])
                    pred,loss,conv_features = sess.run([logits,loss_tensor,conv_output],feed_dict={ x: t_input, y: t_label})
