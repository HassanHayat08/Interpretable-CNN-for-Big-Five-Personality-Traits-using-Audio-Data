# Interpretable cnn for big five persoanlity traits #
# Get conv_features and corresponding input spectrogram and output layer weights #
  ...
  with tf.Graph().as_default():
       ...
       with slim.arg_scope([slim.conv2d, slim.fully_connected],
         ...
         tf.variable_scope('cnn'):
           ...
           # Ouput of last conv Layer.
           output = tf.identity(net, name='conv_output')
           # Global Average Pooling layer.
           net = slim.avg_pool2d(net, [params.kernel_size_x,params.kernel_size_y],scope='GAP')
           # Flatten the last output for fully-connected layers.
           net = slim.flatten(net)
           # Output layer.
           logits = slim.fully_connected(net,param.NUM_CLASSES, activation_fn=None,trainable=False, scope='logits')
       ...
       with tf.Session() as sess:
         # Locate the tensors for training.
         feature_tensor = sess.graph.get_tensor_by_name('cnn/mfcc_features')
         label_tensor = sess.graph.get_tensor_by_name('cnn/model/labels:0')
         conv_output = sess.graph.get_tensor_by_name('cnn/conv_output:0')

         # Load trained model weights.
         weight_file = '.../path/to/load/model/weights/CAM_Clip_weights.npz'
         skip_layer = []
